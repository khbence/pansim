...
namespace IOProgression {
    ...
    public:
        explicit TooMuchBad(unsigned state)
            : TransitionInputError(
                std::to_string(state) + ". state has multiple bad transition, which is not allowed in this setup.\n") {}
    };
    ...
    public:
        explicit BadChances(const std::string& state, double value)
            : TransitionInputError(
                "Sum of transition chances of state " + state + " is not 1 it is " + std::to_string(value) + ".\n") {}
    };
    ...
    public:
        explicit MissingStateName(const std::string& name)
            : TransitionInputError(
                "State called " + name + " is missing from the file, but it should be there according to the logic.\n") {}
    };
}// namespace IOProgression
...
namespace IOAgents {
    ...
    public:
        explicit NotDefinedAge(unsigned age)
            : AgentsInputError("Age " + std::to_string(age) + " was not defined in the parameter input file!\n") {}
    };
    ...
    public:
        UnnecessaryLocType(unsigned agentID, unsigned aTypeID, unsigned lTypeID)
            : AgentsInputError("Agent with the index of " + std::to_string(agentID) + " with the agent type ID of "
                               + std::to_string(aTypeID) + " does not need a location type with the ID of "
    ...
    public:
        MissingLocationType(unsigned agentID, unsigned agentType, std::string&& missingTypes)
            : AgentsInputError("Agent with the index of " + std::to_string(agentID) + " type " + std::to_string(agentType)
...
namespace IOAgentTypes {
    ...
    public:
        explicit RepetitiveEventType([[maybe_unused]] const std::string& agentType)
...
namespace init {
    ...
    public:
        explicit BadTimeStep(unsigned timeStep)
            : ProgramInit("Time step of " + std::to_string(timeStep)
