...
public:
    ...
    public:
        ...
        GlobalCondition(std::vector<unsigned> h, bool a, std::function<bool(GlobalCondition*, std::vector<unsigned>&)> r)
            : headerPos(h), active(a), condition(r), pos(0) {}
    };
    ...
    public:
        ...
        Rule(std::string n, std::vector<GlobalCondition*> c, std::function<void(Rule*)> r)
            : name(n), conditions(c), rule(r), previousOpenState(true) {}
    };
        ...
        for (const parser::ClosureRules::Rule& rule : rules.rules) {
            ...
            if (rule.conditionType.compare("afterDays") == 0) {
                ...
                if (closeAfter >= 0 || openAfter <= 0)
                    throw CustomErrors("For closure rule 'afterDays', closeAfter must be -1, openAfter must be >0");
                globalConditions.emplace_back(
                    none, false, [openAfter, threshold](GlobalCondition* c, std::vector<unsigned>& stats) {
                        int day = c->history[0]++;
                        return (day >= threshold && day < threshold + openAfter);
                    });
                ...
                if (closeAfter <= 0 || openAfter <= 0)
                    throw CustomErrors("For closure rule 'newDeadFraction', closeAfter and openAfter must be >0");
            ...
            if (rule.name.compare("Masks") == 0) {
                    ...
                    if (r->previousOpenState != shouldBeOpen) {
                        thrust::for_each(
                            ...
                            [maskCoefficient2, homeType, shouldBeOpen] HD(
                                ...
                                if (type != homeType) {
                                    if (shouldBeOpen)
                                        infectiousness = infectiousness / maskCoefficient2;
                                    else
                                        infectiousness = infectiousness * maskCoefficient2;
                                }
                            });
                ...
                } else if (rule.parameter.length() > 0)
                    throw CustomErrors("curfew parameter string needs to be exactly 9 characters long");
                ...
                this->rules.emplace_back(rule.name, conds, [&, realThis, diags, ageLimit](Rule* r) {
                    ...
                    if (r->previousOpenState != shouldBeOpen) {
                        ...
                        r->previousOpenState = shouldBeOpen;
                        if (diags > 0)
                            printf("School age restriction %s - only under %d years go to school\n",
                                (int)shouldBeOpen ? "disabled" : "enabled",
                                ageLimit);
                    }
                });
                ...
                if (locationTypesToClose.size() >= 10)
                    throw CustomErrors("Error, Closure Rule " + rule.name
                                       + " has over 10 location types to close, please increase implementation limit");
                for (unsigned i = 0; i < locationTypesToClose.size(); i++) { fixListArr[i] = locationTypesToClose[i]; }
                for (unsigned i = locationTypesToClose.size(); i < 10; i++)
                    fixListArr[i] = (typename SimulationType::TypeOfLocation_t) - 1;

                // printf("cond %p\n",&globalConditions[globalConditions.size()-1]);
                std::vector<GlobalCondition*> conds = { &globalConditions[globalConditions.size() - 1] };
                this->rules.emplace_back(rule.name, conds, [&, fixListArr, diags](Rule* r) {
                    bool close = true;
                    for (GlobalCondition* c : r->conditions) {
                        close = close && c->active; /*printf("rule %s cond %p\n", r->name.c_str(), c);*/
                    }
                    // printf("Rule %s %d\n", r->name.c_str(), close ? 1 : 0);
                    bool shouldBeOpen = !close;
                    if (r->previousOpenState != shouldBeOpen) {
                        thrust::for_each(thrust::make_zip_iterator(thrust::make_tuple(
                                             locTypes.begin(), locationOpenState.begin(), locationEssential.begin())),
                            thrust::make_zip_iterator(
                                thrust::make_tuple(locTypes.end(), locationOpenState.end(), locationEssential.end())),
                            [fixListArr, shouldBeOpen] HD(
                                thrust::tuple<typename SimulationType::TypeOfLocation_t&, bool&, uint8_t&> tup) {
                                auto& type = thrust::get<0>(tup);
                                ...
                                for (unsigned i = 0; i < 10; i++)
                                    if (type == fixListArr[i])
                                        isOpen = shouldBeOpen;
                                    else if ((typename SimulationType::TypeOfLocation_t) - 1 == fixListArr[i])
                                        break;
                            });
                        r->previousOpenState = shouldBeOpen;
    ...
    void midnight(Timehandler simTime, unsigned timeStep, std::vector<unsigned>& stats) {
        for (GlobalCondition& c : globalConditions) { c.active = c.condition(&c, stats); }
        for (Rule& r : rules) { r.rule(&r); }
    }
    void step(Timehandler simTime, unsigned timeStep) {}
};
