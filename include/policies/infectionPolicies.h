...
private:
    ...
    public:
        ...
        void insert(unsigned _timestamp,
            unsigned _variant,
            thrust::host_vector<unsigned> _locationIds,
            thrust::host_vector<unsigned> _infectedAgentsAtLocation,
            ...
            infectedAgentsAtLocation.insert(infectedAgentsAtLocation.end(),
                _infectedAgentsAtLocation.begin(),
                _infectedAgentsAtLocation.end());
            infectedAgentOffsets.push_back(infectedAgentsAtLocation.size());
            infectiousAgentsAtLocation.insert(infectiousAgentsAtLocation.end(),
                _infectiousAgents.begin(),
                _infectiousAgents.end());
            infectiousAgentOffsets.push_back(infectiousAgentsAtLocation.size());
            infectiousnessOfAgents.insert(infectiousnessOfAgents.end(),
                _infectiousnessOfAgents.begin(),
                _infectiousnessOfAgents.end());
        }
        void write(std::string fname) {
            ...
            file << timestamp.size() << "\n";
            thrust::copy(
                timestamp.begin(), timestamp.end(), std::ostream_iterator<unsigned>(file, " "));
            file << "\n";
            thrust::copy(
                variant.begin(), variant.end(), std::ostream_iterator<unsigned>(file, " "));
            file << "\n";
            ...
            file << "\n";
            thrust::copy(
                locationIds.begin(), locationIds.end(), std::ostream_iterator<unsigned>(file, " "));
            file << "\n";
            thrust::copy(infectedAgentOffsets.begin(),
                infectedAgentOffsets.end(),
                std::ostream_iterator<unsigned>(file, " "));
            file << "\n";
            thrust::copy(infectedAgentsAtLocation.begin(),
                infectedAgentsAtLocation.end(),
                std::ostream_iterator<unsigned>(file, " "));
            file << "\n";
            thrust::copy(infectiousAgentOffsets.begin(),
                infectiousAgentOffsets.end(),
                std::ostream_iterator<unsigned>(file, " "));
            file << "\n";
            thrust::copy(infectiousAgentsAtLocation.begin(),
                infectiousAgentsAtLocation.end(),
                std::ostream_iterator<unsigned>(file, " "));
            file << "\n";
            thrust::copy(infectiousnessOfAgents.begin(),
                infectiousnessOfAgents.end(),
...
public:
    static void addProgramParameters(cxxopts::Options& options) {
        options.add_options()("k,infectionCoefficient",
            ...
            cxxopts::value<double>()->default_value("0.000362213"))(
            "dumpLocationInfections",// 291642
    ...
    void finalize() {
        if (flagInfectionAtLocations) dumpstore->write(dumpDirectory);
        delete dumpstore;
    }
...
public:
    ...
    void dumpToFileStep1(thrust::device_vector<unsigned>& locationListOffsets,
        ...
        if (dumpToFile > 0) {// Aggregate new infected counts
            reduce_by_location(locationListOffsets,
                ...
                ppstates,
                agentLocations,
                [variant] HD(const typename SimulationType::PPState_t& ppstate) -> unsigned {
                    return ppstate.getSusceptible(variant) > 0;
                });
        }
        if (dumpToFile > 0 && simTime.getTimestamp() % dumpToFile == 0) {
            file.open("locationStats_v" + std::to_string(variant) + "_"
                      + std::to_string(simTime.getTimestamp()) + ".txt");
    ...
    void dumpLocationInfectiousList(thrust::device_vector<PPState_t>& ppstates,
        ...
        auto getAgentIDFromLocPair = [] HD(thrust::pair<unsigned, unsigned> val) {
            return val.second;
        };
        thrust::for_each(
            thrust::make_zip_iterator(newlyInfectedAgentsByLocation.begin(),
                // thrust::make_transform_iterator(locationAgentList.begin(),
            ...
            thrust::make_zip_iterator(newlyInfectedAgentsByLocation.end(),
                // thrust::make_transform_iterator(locationAgentList.end(),
                //     [] HD(thrust::pair<unsigned, unsigned> val) { return val.second; }),
                locationAgentList.end(),
                thrust::make_permutation_iterator(
                    newlyInfectedPeopleIds.begin(), newlyInfectedAgentOffsetsByLocation.end())),
            [] HD(thrust::tuple<unsigned, thrust::pair<unsigned, unsigned>, unsigned&> tup) {
                if (thrust::get<0>(tup))// if person should be written
                    thrust::get<2>(tup) = thrust::get<1>(tup).second;// then write ID there
            });
        ...
        thrust::for_each(
            thrust::make_zip_iterator(thrust::make_tuple(peopleFlagsByLocation.begin(),
                thrust::make_permutation_iterator(ppstates.begin(), locationAgentList.begin()),
                thrust::make_permutation_iterator(
                    infectiousness.begin(), peopleOffsetsByLocation.begin()))),
            thrust::make_zip_iterator(thrust::make_tuple(peopleFlagsByLocation.end(),
                thrust::make_permutation_iterator(ppstates.begin(), locationAgentList.end()),
                thrust::make_permutation_iterator(
                    infectiousness.begin(), peopleOffsetsByLocation.end()))),
        ...
        /*std::ofstream file;
        file.open(dumpDirectory + "infectiousList_v" + std::to_string(variant) + "_" +
        std::to_string(simTime.getTimestamp())
                  + ".txt");
        ...
        thrust::copy(newlyInfectedPeopleIds.begin(), newlyInfectedPeopleIds.end(),
        std::ostream_iterator<unsigned>(file, " ")); file << "\n";
        thrust::copy(locationLength.begin(), locationLength.end(),
        std::ostream_iterator<unsigned>(file, " ")); file << "\n"; thrust::copy(peopleIds.begin(),
        peopleIds.end(), std::ostream_iterator<unsigned>(file, " ")); file << "\n"; file.close();*/
        dumpstore->insert(simTime.getTimestamp(),
            variant,
            outLocationIds,
            newlyInfectedPeopleIds,
            peopleIds,
            infectiousness);
    }
    ...
    double seasonalityMultiplier(Timehandler& simTime) {
        ...
        double c0 = 2.5;// 0.8;
        int d_mod = d % 366;
        if (d_mod > 366 / 2) d_mod = 366 - d_mod;
        double normed_value =
    ...
    void infectionsAtLocations(Timehandler& simTime, unsigned timeStep, uint8_t variant) {
        ...
        thrust::transform(thrust::make_zip_iterator(thrust::make_tuple(fullInfectedCounts.begin(),
            ...
            [=] HD(thrust::tuple<float&, unsigned&, unsigned&, double&> tuple) {
                ...
                return 1.0
                       - exp(-tmpK * densityOfInfected * thrust::get<3>(tuple) * seasonality
        ...
        dumpToFileStep2(
            locationListOffsets, locationAgentList, ppstates, agentLocations, simTime, variant);
    }
};
