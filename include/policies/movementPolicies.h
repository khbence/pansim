...
public:
    ...
    void movement(Timehandler simTime, unsigned timeStep) {
        //        PROFILE_FUNCTION();
        auto realThis = static_cast<SimulationType*>(this);
        auto& locationAgentList = realThis->locs->locationAgentList;
        ...
        Util::updatePerLocationAgentListsSort(
...
__device__
    ...
    if (a.agentStatsPtr[i].diagnosedTimestamp > 0 && a.agentStatesPtr[i].isInfected() == false)
        return;
        ...
        if (a.quarantinePolicy > 2 && previousQuarantineUntil < a.timestamp) {
            ...
            for (unsigned loc : toClose) {
                if (loc != std::numeric_limits<unsigned>::max()
                    ...
                    && a.locationStatesPtr[loc] == true && a.closedUntilPtr[loc] < a.timestamp) {
                    if (i == a.tracked)
...
__device__
    ...
    if (a.enableCurfew && a.curfewBegin == a.simTime.getMinutes() / a.timeStep) {
        ...
        bool deadOrHospitalized =
            (wBState == states::WBStates::D || wBState == states::WBStates::S);
        bool hospitalizedWithNonCOVID =
    ...
    if (a.agentStatsPtr[i].hospitalizedTimestamp <= a.timestamp
        ...
        a.stepsUntilMovePtr[i] =
            MIN(a.agentStatsPtr[i].hospitalizedUntilTimestamp - a.timestamp - 1,
                a.simTime.getStepsUntilMidnight(a.timeStep));
        a.agentLocationsPtr[i] = RealMovementOps::findActualLocationForType(i,
            a.hospitalType,
            a.locationOffsetPtr,
        ...
        if (i == a.tracked) {
            printf(
                "Agent %d of type %d day %d at %d:%d WBState %d in hospital %d due to non-COVID "
    ...
    if (wBState == states::WBStates::S) {// go to hospital if in serious condition
        ...
        if (!a.diagnosedPtr[i] && a.agentStatesPtr[i].isInfectious()) {
            ...
            RealMovementOps::quarantineAgent(
                i, a, a.timestamp + a.quarantineLength * 24 * 60 / a.timeStep);
        }
        checkLarger(i, a);
        return;
    }
    ...
    if (a.quarantinePolicy > 1 && a.timestamp < a.locationQuarantineUntilPtr[a.agentLocationsPtr[i]]
        ...
        if (a.quarantinedPtr[i] == true) {
            ...
            if (agentHome != std::numeric_limits<unsigned>::max()
                && a.agentLocationsPtr[i] != agentHome && a.stepsUntilMovePtr[i] > 60 / a.timeStep)
            ...
            && (a.diagnosedPtr[i]
                || (a.agentStatsPtr[i].diagnosedTimestamp > 0
                    && (a.timestamp - a.agentStatsPtr[i].diagnosedTimestamp)
                           < a.quarantineLength * 24 * 60 / a.timeStep)))// stay home if diagnosed
                                                                         // or quarantine has not
                                                                         // expired
    ...
    if (activeEventsBegin != -1) {
        unsigned numPotentialEvents =
            (activeEventsEnd == -1 ? activeEventsBegin + 1 : activeEventsEnd) - activeEventsBegin;
        ...
        if (numPotentialEvents == 1) {
            ...
            agentType + 1, wBState, (int)a.day, a.simTime.getMinutes() / 60,
            a.simTime.getMinutes() % 60);*/
                idx--;
            }
            pickedEventIdx = idx;
        ...
        bool schoolAndTooOld =
            (newLocationType == a.schoolType || newLocationType == a.classroomType)
                ...
                double threshhold =
                    (pickedEventIdx == 0)
                        ...
                        : a.eventsPtr[activeEventsBegin].chance
                              / (1.0 - a.eventsPtr[activeEventsBegin + pickedEventIdx].chance);
                unsigned idx = 0;
                while (rand > threshhold && idx < numPotentialEvents) {
                    idx++;
                    threshhold +=
        ...
        if (basicDuration.getHours() > 24) {
            ...
            if (a.simTime + basicDuration < a.eventsPtr[activeEventsEnd].start) {
                ...
                a.stepsUntilMovePtr[i] =
                    (a.eventsPtr[activeEventsEnd].end - a.simTime).steps(a.timeStep) - 1;
                checkLarger(i, a);
            } else {
        ...
        if (agentType + 1 == 7 && a.enableCurfew
            ...
            bool workButClosed = (newLocationType == a.workType
                                  && (a.locationStatesPtr[newLocation] == false
                                      || a.closedUntilPtr[newLocation] > a.timestamp));
    ...
    checkLarger(i, a);
    if (agentHome != std::numeric_limits<unsigned>::max() && a.agentLocationsPtr[i] != agentHome
        && a.stepsUntilMovePtr[i] > 60 / a.timeStep) {
        a.stayedHomePtr[i] = false;
        if (i == a.tracked)
            printf("\tdid not stay home %d %d\n", a.agentLocationsPtr[i], agentHome);
    }
    a.stepsUntilMovePtr[i]--;
}
...
__device__
    ...
    if (quarantinedPtr[i]) {
        ...
        unsigned schoolLocation = RealMovementOps::findActualLocationForType(i,
            school,
            locationOffsetPtr,
            possibleLocationsPtr,
            possibleTypesPtr,
            ...
            0,
            nullptr);
        unsigned classroomLocation = RealMovementOps::findActualLocationForType(i,
            classroom,
            locationOffsetPtr,
            possibleLocationsPtr,
            possibleTypesPtr,
...
__device__
    ...
    if (agentMetaDataPtr[i].getAge() > 26 && agentMetaDataPtr[i].getAge() < 65) {
        ...
        if (homeLocation != std::numeric_limits<unsigned>::max())
            if (noWorkLocPtr[homeLocation]
                == 1) {// TODO this is not exactly thread safe on the CPU....
#if THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
...
__device__
    ...
    unsigned workLocation = RealMovementOps::findActualLocationForType(i,
        work,
        locationOffsetPtr,
        possibleLocationsPtr,
        possibleTypesPtr,
...
__device__
    ...
    unsigned counter = 0;
    unsigned max = 0;
    for (unsigned classOffset = classroomOffsetsPtr[i]; classOffset < classroomOffsetsPtr[i + 1];
         classOffset++) {
    ...
    if (counter > 1) {
        ...
        for (unsigned classOffset = classroomOffsetsPtr[i];
             classOffset < classroomOffsetsPtr[i + 1];
             classOffset++) {
            unsigned classIdx = classroomsPtr[classOffset];
...
__global__ void checkSchoolQuarantineDriver(unsigned numSchools,
    ...
    unsigned i = threadIdx.x + blockIdx.x * blockDim.x;
    if (i < numSchools) {
        RealMovementOps::checkSchoolQuarantine(i,
            schoolsPtr,
            classroomsPtr,
            classroomOffsetsPtr,
            locationQuarantineUntilPtr,
...
class RealMovement {
    ...
    thrust::device_vector<uint8_t>
        noWorkAgent;// indicating agent staying home because children at home
...
public:
    ...
    static void addProgramParameters(cxxopts::Options& options) {
        options.add_options()("trace",
            ...
            cxxopts::value<unsigned>()->default_value(
                std::to_string(unsigned(3))))("quarantineLength",
            "Length of quarantine in days",
            cxxopts::value<unsigned>()->default_value(
                std::to_string(unsigned(10))))("dumpLocationAgentList",
    ...
    void planLocations(Timehandler simTime, unsigned timeStep) {
        ...
        thrust::fill(
            realThis->agents->stayedHome.begin(), realThis->agents->stayedHome.end(), true);

        // For each agent that is under 14 years, check if quarantined or school closed, if so flag
        // home as noWork
        ...
        typename SimulationType::AgentMeta_t* agentMetaDataPtr =
            thrust::raw_pointer_cast(agentMetaData.data());
        ...
        typename SimulationType::PPState_t* agentStatesPtr =
            thrust::raw_pointer_cast(agentStates.data());
        ...
        // if quarantinePolicy >3, check if more than 1 classrrom in a school is quarantined, if so,
        // quarantine the schooland all classrooms
...
#pragma omp parallel for
        ...
        // put all attendees of quarantined classroom into quarantine, clear classroom quarantine,
        // close classroom
...
#pragma omp parallel for
...
#elif THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
            RealMovementOps::
                checkSchoolWorkQuarantineDriver<<<(numberOfAgents - 1) / 256 + 1, 256>>>(
                    numberOfAgents,
                    agentStatsPtr,
                    agentStatesPtr,
                    quarantinedPtr,
            ...
            thrust::for_each(
                ...
                [schoolType, classroomType, workType, timestamp] HD(
                    thrust::tuple<unsigned&, unsigned&, typename SimulationType::TypeOfLocation_t&>
                        tup) {
                    if (thrust::get<1>(tup) > timestamp
                        && (thrust::get<2>(tup) == schoolType
                            || thrust::get<2>(tup) == classroomType)) {
...
#pragma omp parallel for
        ...
        // For each adult working agent (25-65), if home is flagged, at least one adult is flagged
        // as not working that day
#if THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_OMP
#pragma omp parallel for
        for (unsigned i = 0; i < numberOfAgents; i++) {
            RealMovementOps::setNoWorkToday(i,
...
#pragma omp parallel for
        ...
        if (dumpLocationAgentList.length() > 0) {
            std::ofstream file;
            file.open(
                dumpLocationAgentList + "/locationList" + std::to_string(a.timestamp) + ".txt");
            ...
            file << "\n";
            // here for some reason a plain thrust::transform doesn't work
            auto helper = []HD (thrust::pair<unsigned, unsigned> e) {
                    return e.second;
                };
            thrust::copy(thrust::make_transform_iterator(locationAgentList.begin(), helper),
                thrust::make_transform_iterator(locationAgentList.end(), helper),
                std::ostream_iterator<unsigned>(file, " "));
            file << "\n";
            file.close();
        }
