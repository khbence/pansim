...
namespace policies {
namespace movement {
class AbstractMovement : public AbstractPolicy {
    void init(const nlohmann::json& data, unsigned cemeteryID);
...
namespace mvmpol = policies::movement;
...
//         realThis->locs->locationListOffsets; thrust::device_vector<unsigned>& locationIdsOfAgents
//         = realThis->locs->locationIdsOfAgents; thrust::device_vector<unsigned>& agentLocations =
//         realThis->agents->location; unsigned numberOfAgents = agentLocations.size(); unsigned
//         numberOfLocations = locationListOffsets.size() - 1;

//         thrust::for_each(thrust::make_zip_iterator(thrust::make_tuple(agentLocations.begin(),
//         stepsUntilMove.begin())),
//             thrust::make_zip_iterator(thrust::make_tuple(agentLocations.end(),
//             stepsUntilMove.end())), [numberOfLocations] HD(thrust::tuple<unsigned&, unsigned&>
//             tuple) {
...
//                     stepsUntilMove = RandomGenerator::randomUnsigned(144 / 4);// Move 4 times per
//                     day on average
//                 }
//                 stepsUntilMove--;
//             });
//         Util::updatePerLocationAgentListsSort(agentLocations, locationIdsOfAgents,
//         locationAgentList, realThis->locs->locationPartAgentList, locationListOffsets);
//     }
// };
...
//         }
//         if (counter == 1)
//             return possibleLocationsPtr[pos];
//         else if (counter > 1) {
//             // Pick one at random
...
//                     if (locType == possibleTypesPtr[i]) {
//                         if (counter2 == randIdx) {
//                             if (retry && locationStatesPtr[possibleLocationsPtr[i]] == false) {
//                                 retry = 0;
//                                 break;
//                             } else
...
// #endif
//         void
//         quarantineAgent(unsigned i, MovementArguments<PPState, AgentMeta, LocationType>& a,
//         unsigned until, bool quarantineImmuneActive) { if (a.quarantinePolicy == 0) return; if
//         (a.agentStatsPtr[i].diagnosedTimestamp > 0 && a.agentStatesPtr[i].isInfected() == false
//         && quarantineImmuneActive == false) return; if (a.agentStatsPtr[i].immunizationTimestamp
//         > 0 && quarantineImmuneActive == false) return; a.quarantinedPtr[i] = true;
...
//                         && (a.locationTypePtr[loc] == a.workType || a.locationTypePtr[loc] ==
//                         a.classroomType)
...
//                         if (i == a.tracked) printf("\tFlagging work/classroom as quarantined:
//                         %d\n", loc); a.locationQuarantineUntilPtr[loc] = until;// TODO:
//                         quarantine period
//                     }
//                 }
//             }
...
//             bool deadOrHospitalized = (wBState == states::WBStates::D || wBState ==
//             states::WBStates::S); bool hospitalizedWithNonCOVID =
...
//                         nullptr);
//                     if (a.tracked == i)
//                         printf("Agent %d day %d at %d:%d WBState %d moved to home %d due to
//                         curfew\n",
//                             i,
//                             (int)a.day,
...
//         if (a.agentStatsPtr[i].quarantinedUntilTimestamp <= a.timestamp) { a.quarantinedPtr[i] =
//         false; }
...
//         unsigned agentHome = RealMovementOps::findActualLocationForType(i,
//             a.homeType,
//             a.locationOffsetPtr,
//             a.possibleLocationsPtr,
//             a.possibleTypesPtr,
...
//             && a.agentStatsPtr[i].hospitalizedUntilTimestamp > a.timestamp && wBState !=
//             states::WBStates::S
//             && wBState != states::WBStates::D) {

//             a.stepsUntilMovePtr[i] = MIN(
//                 a.agentStatsPtr[i].hospitalizedUntilTimestamp - a.timestamp - 1,
//                 a.simTime.getStepsUntilMidnight(a.timeStep));
//             a.agentLocationsPtr[i] = RealMovementOps::findActualLocationForType(i,
//                 a.hospitalType,
//                 a.locationOffsetPtr,
...
//             if (i == a.tracked) {
//                 printf(
//                     "Agent %d of type %d day %d at %d:%d WBState %d in hospital %d due to
//                     non-COVID hospitalization between "
//                     "%d-%d\n",
//                     i,
//                     agentType + 1,
//                     (int)a.day,
...
//                 if (a.simTime.getStepsUntilMidnight(a.timeStep) == 24 * 60 / a.timeStep)// is it
//                 midnight, and agent got S
//                                                                                         // due to
//                                                                                         disease
//                                                                                         progression?
...
//                 RealMovementOps::quarantineAgent(i, a, a.timestamp + a.quarantineLength * 24 * 60
//                 / a.timeStep, a.quarantineImmuneActive);
//             }
//             checkLarger(i, a);
//             return;
//         }
...
//         if (a.quarantinePolicy > 1 && a.timestamp <
//         a.locationQuarantineUntilPtr[a.agentLocationsPtr[i]]
//             && (a.locationTypePtr[a.agentLocationsPtr[i]] == a.homeType
//                 || a.locationTypePtr[a.agentLocationsPtr[i]] == a.schoolType// Only send agent to
//                 quarantine if this
//                                                                             // is home, work or
//                                                                             school
...
//                 a.locationQuarantineUntilPtr[a.agentLocationsPtr[i]], a.quarantineImmuneActive);
//             }
//             // if now quarantined
//             if (a.quarantinedPtr[i] == true) {
//                 a.stepsUntilMovePtr[i] = MIN(a.locationQuarantineUntilPtr[a.agentLocationsPtr[i]]
//                 - a.timestamp - 1,
//                     a.simTime.getStepsUntilMidnight(a.timeStep));
...
//                 unsigned homeLocation = RealMovementOps::findActualLocationForType(i,
//                     a.homeType,
//                     a.locationOffsetPtr,
//                     a.possibleLocationsPtr,
//                     a.possibleTypesPtr,
...
//                 }
//                 checkLarger(i, a);
//                 if (agentHome != std::numeric_limits<unsigned>::max() && a.agentLocationsPtr[i]
//                 != agentHome
...
//                                < a.quarantineLength * 24 * 60 / a.timeStep)))// stay home if
//                                diagnosed or
//                                                                              // quarantine has
//                                                                              not
//                                                                              // expired
...
//                 RealMovementOps::quarantineAgent(i, a, a.timestamp + a.quarantineLength * 24 * 60
//                 / a.timeStep, a.quarantineImmuneActive); if (i == a.tracked &&
//                 a.quarantinedPtr[i]) {
//                     printf(
...
//         // 2 Begin != -1, End == -1 -> last event for the day. Move there (if needed pick
//         randomly)
...
//             newLocationType = wBState == states::WBStates::S ? a.hospitalType : a.homeType;//
//             Hostpital if sick, home otherwise unsigned myHome =
...
//             unsigned numPotentialEvents = (activeEventsEnd == -1 ? activeEventsBegin + 1 :
//             activeEventsEnd) - activeEventsBegin; unsigned pickedEventIdx = 0; TimeDayDuration
//             basicDuration(0.0); if (numPotentialEvents == 1) {
...
//                     /*printf("Error, overrun1: %g, agentType %d WB %d, day %d, time
...
//             // if agent has to stay home with children, then check to see if is work, and set it
//             to home if (a.noWorkAgentPtr[i] != 0 && newLocationType == a.workType) {
...
//             bool schoolAndTooOld = (newLocationType == a.schoolType || newLocationType ==
//             a.classroomType)
//                                    && a.agentMetaDataPtr[i].getAge() >= a.schoolAgeRestriction;
//             if (schoolAndTooOld //school is closed for student
//                 || ((a.locationStatesPtr[newLocation] == false || a.closedUntilPtr[newLocation] >
//                 a.timestamp)
//                     && newLocationType != a.workType) //or destination is closed and it's not
//                     agent's workplace
//                 || (a.lockdownNonvaccActive && a.agentStatsPtr[i].immunizationTimestamp==0)) //or
//                 non-immune should be locked down, and this is nonessential
//                 {
//                 // If closed, but there is another option to go to different type location, try
//                 that if (numPotentialEvents > 1) {
//                     double rand = RandomGenerator::randomReal(1.0);
//                     double threshhold = (pickedEventIdx == 0)
//                                             ? 0.0
//                                             : a.eventsPtr[activeEventsBegin].chance
//                                                   / (1.0 - a.eventsPtr[activeEventsBegin +
//                                                   pickedEventIdx].chance);
//                     unsigned idx = 0;
//                     while (rand > threshhold && idx < numPotentialEvents) {
//                         idx++;
//                         threshhold += (pickedEventIdx == idx)
//                                           ? 0.0
//                                           : a.eventsPtr[activeEventsBegin + idx].chance
//                                                 / (1.0 - a.eventsPtr[activeEventsBegin +
//                                                 pickedEventIdx].chance);
//                     }
//                     if (idx == numPotentialEvents) { idx--; }
//                     newLocationType = a.eventsPtr[activeEventsBegin + idx].locationType;
//                     basicDuration = a.eventsPtr[activeEventsBegin + idx].duration;
...
//                     bool schoolAndTooOld2 = (newLocationType == a.schoolType || newLocationType
//                     == a.classroomType)
//                                             && a.agentMetaDataPtr[i].getAge() >=
//                                             a.schoolAgeRestriction;
//                     // is that closed too?
//                     if (schoolAndTooOld
//                         || ((a.locationStatesPtr[newLocation] == false ||
//                         a.closedUntilPtr[newLocation] > a.timestamp)
//                                && newLocationType != a.workType)
//                         || (a.lockdownNonvaccActive &&
//                         a.agentStatsPtr[i].immunizationTimestamp==0)) { wasClosed = newLocation;
//                         newLocation = RealMovementOps::findActualLocationForType(i,
//                             a.homeType,
//                             a.locationOffsetPtr,
//                             a.possibleLocationsPtr,
...
//                 unsigned myClassroom = RealMovementOps::findActualLocationForType(i,
//                     a.classroomType,
//                     a.locationOffsetPtr,
//                     a.possibleLocationsPtr,
//                     a.possibleTypesPtr,
...
//                     a.stepsUntilMovePtr[i] = (a.eventsPtr[activeEventsEnd].end -
//                     a.simTime).steps(a.timeStep) - 1; checkLarger(i, a);
//                 } else {
//                     // Otherwise I need to move again randomly between the end
//                     // of this duration and the end of next movement window
//                     TimeDayDuration window = a.eventsPtr[activeEventsEnd].end - (a.simTime +
//                     basicDuration); unsigned st = window.steps(a.timeStep); unsigned randExtra =
//                     RandomGenerator::randomUnsigned(st); a.stepsUntilMovePtr[i] =
//                     basicDuration.steps(a.timeStep) + randExtra; checkLarger(i, a);
//                 }
//             }
//             if (agentType + 1 == 7 && a.enableCurfew
//                 && (a.curfewBegin <= a.simTime.getMinutes() / a.timeStep
//                     || a.curfewEnd > a.simTime.getMinutes() / a.timeStep)) {
//                 bool workButClosed =
//                     (newLocationType == a.workType
//                         && (a.locationStatesPtr[newLocation] == false ||
//                         a.closedUntilPtr[newLocation] > a.timestamp));
//                 if (workButClosed || (newLocationType != a.workType && newLocationType !=
//                 a.homeType)) {
//                     a.agentLocationsPtr[i] = agentHome;
//                     if (i == a.tracked) {
//                         printf(
//                             "\tCase 2&4- Night shift worker tried moving to locType %d location
//                             %d, " "but it's curfew, moving home to %d for %d steps\n",
//                             newLocationType,
//                             newLocation,
//                             agentHome,
//                             a.stepsUntilMovePtr[i] - 1);
...
//             a.eventsPtr[activeEventsEnd].start; unsigned length_steps = length.steps(a.timeStep);
//             unsigned randDelay = RandomGenerator::randomUnsigned(length_steps);
//             a.stepsUntilMovePtr[i] = (a.eventsPtr[activeEventsEnd].start -
//             a.simTime).steps(a.timeStep) + randDelay; unsigned timeLeft = a.stepsUntilMovePtr[i];
...
//                     a,
//                     a.timestamp + a.quarantineLength * 24 * 60 / a.timeStep,
//                     a.quarantineImmuneActive);// TODO: quarantine period
...
// #if THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
//     template<typename PPState, typename AgentMeta, typename LocationType>
//     __global__ void doMovementDriver(unsigned numberOfAgents, MovementArguments<PPState,
//     AgentMeta, LocationType> a) {
...
// #endif
//         void
//         checkUnderageAtHome(unsigned i,
//             unsigned* noWorkPtr,
//             AgentMeta* agentMetaDataPtr,
//             bool* quarantinedPtr,
//             bool* locationStatesPtr,
...
//                 i, home, locationOffsetPtr, possibleLocationsPtr, possibleTypesPtr, home, school,
//                 classroom, 0, nullptr);
//             if (homeLocation != std::numeric_limits<unsigned>::max()) noWorkPtr[homeLocation] =
//             1;
//         } else {
//             // Check if school open/closed
//             unsigned schoolLocation = RealMovementOps::findActualLocationForType(
//                 i, school, locationOffsetPtr, possibleLocationsPtr, possibleTypesPtr, home,
//                 school, classroom, 0, nullptr);
//             unsigned classroomLocation = RealMovementOps::findActualLocationForType(
//                 i, classroom, locationOffsetPtr, possibleLocationsPtr, possibleTypesPtr, home,
//                 school, classroom, 0, nullptr);
...
//                     closedUntilPtr[schoolLocation] > timestamp))
//                 || (classroomLocation != std::numeric_limits<unsigned>::max()
//                     && (locationStatesPtr[classroomLocation] == false
//                         || closedUntilPtr[classroomLocation] > timestamp))) {// School closed
//                 unsigned homeLocation = RealMovementOps::findActualLocationForType(
//                     i, home, locationOffsetPtr, possibleLocationsPtr, possibleTypesPtr, home,
//                     school, classroom, 0, nullptr);
//                 if (homeLocation != std::numeric_limits<unsigned>::max()) noWorkPtr[homeLocation]
//                 = 1;
//             }
...
//         unsigned i = threadIdx.x + blockIdx.x * blockDim.x;
//         if (i < numberOfAgents) {
//             RealMovementOps::checkUnderageAtHome(i,
//                 noWorkPtr,
//                 agentMetaDataPtr,
//                 quarantinedPtr,
//                 locationStatesPtr,
...
//             unsigned homeLocation = RealMovementOps::findActualLocationForType(
//                 i, home, locationOffsetPtr, possibleLocationsPtr, possibleTypesPtr, home, home,
//                 home, 0, nullptr);
//             if (homeLocation != std::numeric_limits<unsigned>::max())
//                 if (noWorkLocPtr[homeLocation] == 1) {// TODO this is not exactly thread safe on
//                 the CPU....
// #if THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
...
// #endif
//         void
//         checkSchoolWorkQuarantine(unsigned i,
//             AgentStats* agentStatsPtr,
//             PPValues* agentStatesPtr,
//             bool* quarantinedPtr,
//             unsigned* locationQuarantineUntilPtr,
//             unsigned long* locationOffsetPtr,
//             unsigned* possibleLocationsPtr,
...
//         if (agentStatsPtr[i].diagnosedTimestamp > 0 && agentStatesPtr[i].isInfected() == false)
//         return;
//         // Get work/school/classroom ID
//         unsigned workLocation = RealMovementOps::findActualLocationForType(
//             i, work, locationOffsetPtr, possibleLocationsPtr, possibleTypesPtr, home, school,
//             classroom, 0, nullptr);
//         unsigned schoolLocation = RealMovementOps::findActualLocationForType(
//             i, school, locationOffsetPtr, possibleLocationsPtr, possibleTypesPtr, home, school,
//             classroom, 0, nullptr);
//         unsigned classroomLocation = RealMovementOps::findActualLocationForType(
//             i, classroom, locationOffsetPtr, possibleLocationsPtr, possibleTypesPtr, home,
//             school, classroom, 0, nullptr);
...
//         unsigned i = threadIdx.x + blockIdx.x * blockDim.x;
//         if (i < numberOfAgents) {
//             RealMovementOps::checkSchoolWorkQuarantine(i,
//                 agentStatsPtr,
//                 agentStatesPtr,
//                 quarantinedPtr,
...
//         unsigned counter = 0;
//         unsigned max = 0;
//         for (unsigned classOffset = classroomOffsetsPtr[i]; classOffset < classroomOffsetsPtr[i +
//         1]; classOffset++) {
//             unsigned classIdx = classroomsPtr[classOffset];
//             if (locationQuarantineUntilPtr[classIdx] > timestamp) {
//                 counter++;
//                 max = max > locationQuarantineUntilPtr[classIdx] ? max :
//                 locationQuarantineUntilPtr[classIdx];
//             }
//         }
//         if (counter > 1) {
//             // printf("School %d has %d quarantined classes, quarantining entire school until
//             %d\n", schoolIdx, counter, max); locationQuarantineUntilPtr[schoolIdx] = max; for
//             (unsigned classOffset = classroomOffsetsPtr[i]; classOffset < classroomOffsetsPtr[i +
//             1]; classOffset++) {
//                 unsigned classIdx = classroomsPtr[classOffset];
...
//                 i, schoolsPtr, classroomsPtr, classroomOffsetsPtr, locationQuarantineUntilPtr,
//                 timestamp);
//         }
//     }
// #endif
...
//     thrust::device_vector<uint8_t> noWorkAgent;// indicating agent staying home because children
//     at home unsigned publicSpace; unsigned home; unsigned hospital; unsigned cemeteryLoc;
//     unsigned doctor;
//     unsigned tracked;
//     unsigned quarantineLength;
//     unsigned school;
...
//                 "threshold", "threshold to switch between sort or set solution",
//                 cxxopts::value<double>()->default_value("0.0")
//             )("performanceRatio", "Ratio for performance, ask Bence for details",
...
//         // needLocationAgentList = !((dumpLocationAgentList == "") && (dumpInf == 0) &&
//         (dumInfList == "")); needLocationAgentList = true; // for the benchmark
//     }
//     void init(const parser::LocationTypes& data, unsigned cemeteryID) {
...
//         thrust::fill(realThis->agents->stayedHome.begin(), realThis->agents->stayedHome.end(),
//         true);

//         // For each agent that is under 14 years, check if quarantined or school closed, if so
//         flag home as noWork thrust::fill(noWorkLoc.begin(), noWorkLoc.end(), (uint8_t)0u);
...
//         realThis->agents->agentMetaData; typename SimulationType::AgentMeta_t* agentMetaDataPtr =
//         thrust::raw_pointer_cast(agentMetaData.data()); thrust::device_vector<bool>& quarantined
//         = realThis->agents->quarantined; bool* quarantinedPtr =
//         thrust::raw_pointer_cast(quarantined.data()); thrust::device_vector<bool>& locationStates
//         = realThis->locs->states; bool* locationStatesPtr =
//         thrust::raw_pointer_cast(locationStates.data()); thrust::device_vector<unsigned>&
//         closedUntil = realThis->locs->closedUntil; unsigned* closedUntilPtr =
//         thrust::raw_pointer_cast(closedUntil.data()); thrust::device_vector<unsigned long>&
//         locationOffset = realThis->agents->locationOffset; unsigned long* locationOffsetPtr =
//         thrust::raw_pointer_cast(locationOffset.data()); thrust::device_vector<unsigned>&
//         possibleLocations = realThis->agents->possibleLocations; unsigned* possibleLocationsPtr =
//         thrust::raw_pointer_cast(possibleLocations.data()); thrust::device_vector<unsigned>&
//         possibleTypes = realThis->agents->possibleTypes; unsigned* possibleTypesPtr =
//         thrust::raw_pointer_cast(possibleTypes.data()); thrust::device_vector<typename
//         SimulationType::PPState_t>& agentStates = realThis->agents->PPValues; typename
...
//         realThis->locs->locType; thrust::device_vector<unsigned>& locationQuarantineUntil =
//         realThis->locs->quarantineUntil; unsigned* locationQuarantineUntilPtr =
//         thrust::raw_pointer_cast(locationQuarantineUntil.data());

//         // if quarantinePolicy >3, check if more than 1 classrrom in a school is quarantined, if
//         so, quarantine the schooland
//         // all classrooms
...
//             for (unsigned i = 0; i < numSchools; i++) {
//                 RealMovementOps::checkSchoolQuarantine(
//                     i, schoolsPtr, classroomsPtr, classroomOffsetsPtr,
//                     locationQuarantineUntilPtr, timestamp);
//             }
// #elif THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
//             RealMovementOps::checkSchoolQuarantineDriver<<<(numSchools - 1) / 256 + 1, 256>>>(
//                 numSchools, schoolsPtr, classroomsPtr, classroomOffsetsPtr,
//                 locationQuarantineUntilPtr, timestamp);
//             cudaDeviceSynchronize();
// #endif
//         }

//         // put all workers of quarantined workpalces into quarantine, and clear workplace
//         quarantine
//         // put all attendees of quarantined classroom into quarantine, clear classroom
//         quarantine, close classroom if (quarantinePolicy > 2) {
// #if THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_OMP
// #pragma omp parallel for
...
//             }
// #elif THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
//             RealMovementOps::checkSchoolWorkQuarantineDriver<<<(numberOfAgents - 1) / 256 + 1,
//             256>>>(numberOfAgents,
//                 agentStatsPtr,
//                 agentStatesPtr,
//                 quarantinedPtr,
...
//             unsigned workType = work;
//             // Clear quarantine flags, set closedUntil instead for schools/classrooms
//             thrust::for_each(thrust::make_zip_iterator(thrust::make_tuple(
//                                  closedUntil.begin(), locationQuarantineUntil.begin(),
//                                  locationTypes.begin())),
//                 thrust::make_zip_iterator(
//                     thrust::make_tuple(closedUntil.end(), locationQuarantineUntil.end(),
//                     locationTypes.end())),
//                 [schoolType, classroomType, workType, timestamp] HD(
//                     thrust::tuple<unsigned&, unsigned&, typename
//                     SimulationType::TypeOfLocation_t&> tup) { if (thrust::get<1>(tup) > timestamp
//                         && (thrust::get<2>(tup) == schoolType || thrust::get<2>(tup) ==
//                         classroomType)) { thrust::get<0>(tup) = thrust::get<1>(tup);
//                         thrust::get<1>(tup) = 0;
//                     } else if (thrust::get<1>(tup) > timestamp && thrust::get<2>(tup) ==
//                     workType) {
//                         thrust::get<1>(tup) = 0;
//                     }
//                 });
...
//         }
// #elif THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
//         RealMovementOps::checkUnderageAtHomeDriver<<<(numberOfAgents - 1) / 256 + 1,
//         256>>>(numberOfAgents,
//             noWorkLocPtr,
//             agentMetaDataPtr,
//             quarantinedPtr,
...
//         // For each adult working agent (25-65), if home is flagged, at least one adult is
//         flagged as not working that day
// #if THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_OMP
// #pragma omp parallel for
//         for (unsigned i = 0; i < numberOfAgents; i++) {
//             RealMovementOps::setNoWorkToday(i,
...
//         }
// #elif THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
//         RealMovementOps::setNoWorkTodayDriver<<<(numberOfAgents - 1) / 256 + 1,
//         256>>>(numberOfAgents,
//             noWorkLocPtr,
//             noWorkAgentPtr,
//             agentMetaDataPtr,
...
//         thrust::device_vector<unsigned>& locationListOffsets =
//         realThis->locs->locationListOffsets; unsigned* locationListOffsetsPtr =
//         thrust::raw_pointer_cast(locationListOffsets.data()); thrust::device_vector<unsigned>&
//         locationIdsOfAgents = realThis->locs->locationIdsOfAgents; unsigned*
//         locationIdsOfAgentsPtr = thrust::raw_pointer_cast(locationIdsOfAgents.data());
...
//         realThis->locs->quarantineUntil; a.locationQuarantineUntilPtr =
//         thrust::raw_pointer_cast(locationQuarantineUntil.data()); thrust::device_vector<typename
//         SimulationType::TypeOfLocation_t>& locationTypes = realThis->locs->locType;
...
//         realThis->agents->PPValues; a.agentStatesPtr =
//         thrust::raw_pointer_cast(agentStates.data()); thrust::device_vector<typename
//         SimulationType::AgentMeta_t>& agentMetaData = realThis->agents->agentMetaData;
...
//         realThis->agents->agentTypes.events; a.eventsPtr =
//         thrust::raw_pointer_cast(events.data());
...
//         }
// #elif THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
//         RealMovementOps::doMovementDriver<<<(numberOfAgents - 1) / 256 + 1,
//         256>>>(numberOfAgents, a); cudaDeviceSynchronize();
// #endif

//         if(needLocationAgentList) {
//             auto moved = thrust::count_if(movement.begin(), movement.end(), [] HD(const
//             thrust::tuple<unsigned, unsigned, unsigned>& e){
//                 return thrust::get<1>(e) != thrust::get<2>(e);
//             });
//             // std::cout << static_cast<double>(moved) << ", " <<
//             static_cast<double>(movement.size()) << std::endl; if(static_cast<double>(moved) /
//             static_cast<double>(movement.size()) < perfRatio) {
//                 Util::updatePerLocationAgentListsSet(agentLocations,
//                 realThis->locs->locationPartAgentList, realThis->locs->scanResult,
//                 realThis->locs->flags, movement, realThis->locs->copyOfPairs);
//             } else {
//                 Util::updatePerLocationAgentListsSort(agentLocations, locationIdsOfAgents,
//                 locationAgentList, realThis->locs->locationPartAgentList, locationListOffsets);
//             }
...
//             std::ofstream file;
//             file.open(dumpLocationAgentList +
//             "/locationList"+std::to_string(a.timestamp)+".txt"); file <<
//             locationListOffsets.size()-1 << "\n"; thrust::copy(locationListOffsets.begin(),
//             locationListOffsets.end(), std::ostream_iterator<unsigned>(file, " ")); file << "\n";
//             thrust::copy(locationAgentList.begin(), locationAgentList.end(),
//             std::ostream_iterator<unsigned>(file, " ")); file << "\n"; file.close();
//         }
//     }
// };
