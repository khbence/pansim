function [f,h,hvar,J,Init] = mf_epid_ode_model_8comp(Np)
arguments
    Np = 1
end
%%
...
dS = -beta*(P+I+s.delta*A)*S/Np - nu*S + w*R;
...
dD = s.mu*s.lambda*H;
dR = s.rhoI*(1-s.eta)*I + s.rhoA*A + (1-s.mu)*s.lambda*H + nu*S - w*R;
...
matlabFunction(f_sym,'File','Fn_SLPIAHDR_ode','Vars',{x,u,p,nu});
matlabFunction(h_sym,'File','Fn_SLPIAHDR_out','Vars',{x,u,p});

Ts = 1;

f = {};
f.desc = 'f(x,u,p,v)';
f.val = x_Cas + Ts*Fn_SLPIAHDR_ode(x_Cas,u_Cas,p_Cas,nu_Cas);
...
h.val = Fn_SLPIAHDR_out(x_Cas,u_Cas,p_Cas);
