function [Exp_GP,Var_GP,Cov_xGP,Exp_GP_mean,Var_GP_mean] = GP_Exact_MM_Sym(hyp,u,S)
arguments
    hyp struct
    u (:,1)
    S (:,:)
...
%  Reviewed on 2021. December 09. (2021b)
%
...
% Initialize values
Exp_GP = sym(zeros(nz,1));
Var_GP = sym(zeros(nz,nz));
Cov_xGP = sym(zeros(nw,nz));
Exp_GP_mean = sym(zeros(nz,1));
Var_GP_mean = sym(zeros(nz,1));
...
for a = 1:nz
    ...
    % DIM(N,1)
    EXPONENT_of_33 = -0.5 * sum((Zeta{a} * inv(S + Lambda{a})) .* Zeta{a},2);
    ...
    % DIM(n,1)
    Cov_xGP(:,a) = sum(S * inv(S + Lambda{a}) * (ones(nw,1) * ( q' .* beta{a}' ) .* Zeta{a}'),2);
...
for a = 1:nz
    ...
    for b = nz:-1:a
        ...
        C = M(n1+1:nw,1:n1);
        D = M(n1+1:nw,n1+1:nw);
        Determinant_of_R = det(A) * det(D - C*inv(A)*B) * det(iLambda{a} + iLambda{b});

        % Third quadratic term of the exponent in [2, Eq. (2.54)].
        QUADR_TERM_3_of_2_54_col = 0.5 * sum((Z_col * adjoint(R)/Determinant_of_R * S) .* Z_col,2);  % DIM(Na*Nb,1) [SLOW]
...
u = sym('u',[n,1]);
S = pcz_sym_symmetric('S',n);
assumeAlso(in(u,'real'))
assumeAlso(in(S,'real'))
...
[GP_mean,GP_var,GP_cov,GP_mean_ME,GP_var_ME] = GP_Exact_MM_Sym(hyp,u,S);
[GP_mean_val1,GP_var_val1,GP_cov_val1,GP_mean_ME_val1,GP_var_ME_val1] = ...
    GP_Exact_MM(hyp,u_val,S_val);

GP_mean_fh = matlabFunction(GP_mean,'vars',{u,S});
GP_var_fh = matlabFunction(GP_var,'vars',{u,S});
GP_cov_fh = matlabFunction(GP_cov,'vars',{u,S});
GP_mean_ME_fh = matlabFunction(GP_mean_ME,'vars',{u,S});
GP_var_ME_fh = matlabFunction(GP_var_ME,'vars',{u,S});

GP_mean_val2 = GP_mean_fh(u_val,S_val);
GP_var_val2 = GP_var_fh(u_val,S_val);
GP_cov_val2 = GP_cov_fh(u_val,S_val);
GP_mean_ME_val2 = GP_mean_ME_fh(u_val,S_val);
GP_var_ME_val2 = GP_var_ME_fh(u_val,S_val);

fprintf('(Num) Mean equivalent: GP(x) ~ N(%6.3g,%6.3g)\n',GP_mean_ME_val1,GP_var_ME_val1);
fprintf('(Sym) Mean equivalent: GP(x) ~ N(%6.3g,%6.3g)\n',GP_mean_ME_val2,GP_var_ME_val2);
fprintf('(Num) Moment matching: GP(x) ~ N(%6.3g,%6.3g), Cov(x,GP(x)) = (%6.3g,%6.3g)\n',GP_mean_val1,GP_var_val1,GP_cov_val1);
fprintf('(Sym) Moment matching: GP(x) ~ N(%6.3g,%6.3g), Cov(x,GP(x)) = (%6.3g,%6.3g)\n',GP_mean_val2,GP_var_val2,GP_cov_val2);
...
u_Cas = helper.create_sym('u',[n,1],"CasX",CasX);
[S_Cas,S_half] = helper.create_sym('S',n,"str","sym","r2","Var_half");
...
Timer_i6ua = pcz_dispFunctionName('EMM numerical evaluation');
    [GP_mean_val1,GP_var_val1,GP_cov_val1,GP_mean_ME_val1,GP_var_ME_val1] = ...
        GP_Exact_MM(hyp,u_val,S_val);
    pcz_dispFunction2(sprintf('Mean equivalent: GP(x) ~ N(%6.3g,%6.3g)',GP_mean_ME_val1,GP_var_ME_val1));
    pcz_dispFunction2(sprintf('Moment matching: GP(x) ~ N(%6.3g,%6.3g), Cov(x,GP(x)) = (%6.3g,%6.3g,%6.3g,%6.3g,%6.3g)',GP_mean_val1,GP_var_val1,GP_cov_val1));
pcz_dispFunctionEnd(Timer_i6ua);

Timer_i6ua = pcz_dispFunctionName('EMM CasADi computation');
    [GP_mean,GP_var,GP_cov,GP_mean_ME,GP_var_ME] = GP_Exact_MM_Cas(hyp,u_Cas,S_Cas);
    [GP_mean_val3,GP_var_val3,GP_cov_val3,GP_mean_ME_val3,GP_var_ME_val3] = ...
        pcas_full(Function('f',{u_Cas,S_half},{GP_mean,GP_var,GP_cov,GP_mean_ME,GP_var_ME}),u_val,S_val);
    pcz_dispFunction2(sprintf('Mean equivalent: GP(x) ~ N(%6.3g,%6.3g)',GP_mean_ME_val3,GP_var_ME_val3));
    pcz_dispFunction2(sprintf('Moment matching: GP(x) ~ N(%6.3g,%6.3g), Cov(x,GP(x)) = (%6.3g,%6.3g,%6.3g,%6.3g,%6.3g)',GP_mean_val3,GP_var_val3,GP_cov_val3));
pcz_dispFunctionEnd(Timer_i6ua);

Timer_i6ua = pcz_dispFunctionName('EMM symbolic computation');
    [GP_mean,GP_var,GP_cov,GP_mean_ME,GP_var_ME] = GP_Exact_MM_Sym(hyp,u,S);

    pcz_dispFunction([num2str(toc(Timer_i6ua)) ' [sec] -- symbolic moment calculations terminated.'])
    ...
    pcz_dispFunction2(sprintf('Mean equivalent: GP(x) ~ N(%6.3g,%6.3g)',GP_mean_ME_val2,GP_var_ME_val2));
    pcz_dispFunction2(sprintf('Moment matching: GP(x) ~ N(%6.3g,%6.3g), Cov(x,GP(x)) = (%6.3g,%6.3g,%6.3g,%6.3g,%6.3g)',GP_mean_val2,GP_var_val2,GP_cov_val2));
pcz_dispFunctionEnd(Timer_i6ua);
...
┌ GP_Exact_MM_Sym:309 - EMM numerical evaluation
│   Mean equivalent: GP(x) ~ N(-0.226,  1.65)
│   Moment matching: GP(x) ~ N(-0.0286,  1.81), Cov(x,GP(x)) = (0.00637,-0.0208,-0.0219,0.00237,0.00929)
└ 0.00229 [sec]
 
┌ GP_Exact_MM_Sym:316 - EMM CasADi computation
│   Mean equivalent: GP(x) ~ N(-0.226,  1.65)
│   Moment matching: GP(x) ~ N(-0.0286,  1.81), Cov(x,GP(x)) = (0.00637,-0.0208,-0.0219,0.00237,0.00929)
└ 0.046288 [sec]
 
┌ GP_Exact_MM_Sym:324 - EMM symbolic computation
│   - 172.2959 [sec] -- symbolic moment calculations terminated.
│   Mean equivalent: GP(x) ~ N(-0.226,   NaN)
│   Moment matching: GP(x) ~ N(-0.0286,  1.81), Cov(x,GP(x)) = (0.00637,-0.0208,-0.0219,0.00237,0.00929)
└ 3735.6064 [sec]
