...
for Execution = 1:50
    ...
    for k = 0:Nr_Periods-1
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        Uk = D_(isbetween(D_.Date,Start_Date + k*Tp - 15,Start_Date + (k+1)*Tp + 15),:);
        Uk = groupsummary(Uk,Rstr_Vars,["mean","std"],"TrRate");
        %%% 
        beta_min = min(Uk.mean_TrRate - Uk.std_TrRate);
        beta_max = max(Uk.mean_TrRate + Uk.std_TrRate);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for j = 1:Nr_Periods-k-1
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            [~,Idx_Closest] = min(abs(Uk.mean_TrRate-Uk.std_TrRate - beta_sol(j)));
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

            PM(k+j,:) = cellstr(Uk(Idx_Closest,Rstr_Vars).Variables);
            Rp = policy2table(PM(k+j,:));
            for d = 1:Tp
                Idx = Tp*(k+j-1)+d;
                R(Idx,Rp.Properties.VariableNames) = Rp;
                R.TrRateExp(Idx) = Uk.mean_TrRate(Idx_Closest);
            end
        end
