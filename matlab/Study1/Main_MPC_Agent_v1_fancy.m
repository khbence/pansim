...
Visualize_MPC(R,Iref,Pred,0);

%%

Now = datetime;
Now.Format = "uuuu-MM-dd_HH-mm";
DIR = "/home/ppolcz/Dropbox/Peti/Munka/01_PPKE_2020/PanSim_Results/Result_" + string(Now) + "_T" + num2str(Tp);
mkdir(DIR)
...
for k = 0:Nr_Periods-1
    ...
    for j = 1:Nr_Periods-k-1
        [~,Idx_Closest] = min(abs(T.Beta - beta_sol(j)));
    ...
    for d = 1:Tp
        ...
        S = simout2table_(simout);
        % R.TrRate(Idx) = S.TrRate;
        % S.TrRate = R.TrRate(Idx+1);
        R(Idx+1,S.Properties.VariableNames) = S;

        fig = Visualize_MPC(R,Iref,Pred,Idx+1);
...
writetimetable(R,strrep(filename,"Results_","T_"),"Sheet","Results");
