...
for k = 0:Nr_Periods-1
    ...
    % daysk = (1:Tp) + k*Tp;
    % Sk = S(daysk,:);
    % Date0 = Start_Date + k*Tp;
    % DateN = Start_Date + (k+1)*Tp;
    % Sk = Sk(isbetween(Sk.Date,Date0,DateN-1),:);
    % Dk = D(isbetween(D.Date,min(Sk.Date1),max(Sk.Date2)),:);
    ...
    % lb_ = S_.mean_TrRate - 2*S_.std_TrRate;
    % ub_ = S_.mean_TrRate + 2*S_.std_TrRate;
    ...
    for j = k:Nr_Periods-1
        ...
        [PMj,TrRatesj,DjU] = Intervention_Selector(Dj,Date0,Tp,beta_sol(idx_sol),Rstr_Vars);
        idx_sol = idx_sol + 1;
