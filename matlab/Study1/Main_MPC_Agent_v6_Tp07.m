...
[T_ro,Rstr_Vars] = load_policy_measures;
Idx_Intl = find(T_ro.Intelligent);
...
k0_Policy_Idx = 1;
k0_Policy_measures = T_ro(k0_Policy_Idx,Rstr_Vars).Variables;
k0_Expected_beta = T_ro.Beta(k0_Policy_Idx);
...
R.TrRateBounds = R.TrRate*0 + [0.001 1/3];
R.TrRate(2:end) = NaN;
R.I(2:end) = NaN;
R.Ioff = Ioff;
R.Iref = Iref;
...
% Update LUT at avery Tu'th period
Tu = 4;
for k = 0:Nr_Periods-1
    if mod(k,5) == 0
        T = Update_LUT(T_ro,D,Start_Date+([-1 Tu+1]+k)*Tp);
        T_ro = T;
        beta_min = min(T.Beta);
        beta_max = max(T.Beta);
    end
