...
TrRate_IDX = 2;
[T,d,Dly_Range,Dly_Count,Dly_Mean,Dly_Std,Dly_Median] = Aggregator_F_Statistical(TrRate_IDX);
...
k0_Policy_Idx = 1;
k0_Policy_measures = T(k0_Policy_Idx,policy_varnames).Variables;
...
R.TrRateBounds = movmean(Dly_Range,14);
...
beta_var_lb = mean(reshape(Dly_Range(1:end-1,1),Tp,Nr_Periods),1);
beta_var_ub = mean(reshape(Dly_Range(1:end-1,2),Tp,Nr_Periods),1);

% if Tp > 1
measure_fh = @(x,d) vecnorm(exp(10*(x - d))-1,2,1);
% else
%     measure_fh = @(x,d) abs(exp(10*(x - d))-1);
% end    
...
for k = 0:Nr_Periods-1
    ...
    lb = beta_var_lb(k+1:end);
    ub = beta_var_ub(k+1:end);
    beta_var = helper.new_var('beta',size(beta_guess),1,'str','full','lb',lb,'ub',ub);
    ...
    for j = k:Nr_Periods-1
        ...
        mi = Dly_Mean(j*Tp+1:(j+1)*Tp,:);
        [~,Pmx] = min(measure_fh(mi,beta_sol(idx_sol)));

        % Datej = Start_Date+j*Tp+1;
        % if Datej <= datetime(2021,05,15)
        %     Pmx = 1;
        %     Pmx = Pmx_Mindent_lezar;
        %     warning 'Itt most szemelyesen beleszolok: legyen szigoru'
        % end
        % if datetime(2021,02,15) <= Datej && Datej <= datetime(2021,05,15)
        %     Pmx = 1;
        %     warning 'Itt most szemelyesen beleszolok: legyen szigoru'
        % end

        PMj = T(Pmx,policy_varnames).Variables;
        Betas = mi(:,Pmx);
