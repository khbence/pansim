...
p_Iref = helper.new_par('Iref',[1,N]);
p_x0 = helper.new_par('x0',size(x0));
p_Selector = helper.new_par('Selector',[Tp,Nr_Periods]);
p_Param = helper.new_par('Param',[J.np,N]);
helper.gen_par_mfun('Fn_par_MPC_v4',fp.dir)

x_var = helper.new_var('x',[J.nx,N],1,'str','full','lb',0);
x = [p_x0 , x_var];

beta_min = min(T.Beta);
beta_max = max(T.Beta);
beta_var = helper.new_var('beta',[1,Nr_Periods],1,'str','full','lb',beta_min,'ub',beta_max);
beta = beta_var * M;

% Enforce the state equations
for i = 1:N
    x_kp1 = f.Fn(x(:,i),p_Param(:,i),beta(i),0,0);
    helper.add_eq_con( (x_kp1-x(:,i+1)) * p_Selector(i) );
end

% Minimize the tracking error
h = [2 2 3]*N/7;
wIerr = ([ ones(1,h(1)) , Epid_Par.Sigmoid(1,0,h(2)) , zeros(1,h(3)) ] + 0.1) .* p_Selector(:)';
helper.add_obj('I_error',(x_var(J.I,:) - p_Iref).^2,wIerr(1:N_MPC));
...
for k = 0:Nr_Periods-1
    ...
    p_Iref = helper.new_par('Iref',size(Iref));
    p_x0 = helper.new_par('x0',size(x0));
    p_Selector = helper.new_par('Selector',size(Iref));
    p_Param = helper.new_par('Param',size(P.Param(1:N_MPC+1,:)));
