...
Plot_Height = 180;
Spacing = 10;
FigDim = @(Idx) [Plot_Width*2+Spacing Plot_Height*3];
...
arguments
    ...
    Height1 = height_Top + height_Pd1 + 2*(height_Iq + height_I + height_Pd) - height_Pd;
    Height2 = 3*(height_Iq + height_I + height_Pd) - height_Pd;

    Tl_Dim = {Height1 , 2*width_Pl+width_Sp};
    Tl = tiledlayout(Tl_Dim{:},"TileSpacing","none","Padding","none","TileIndexing","columnmajor");
        
    ax = nexttile([height_Top,width_Pl]);
    ...
    text(-0.2,2.8,'Overall strictness of interventions:','Interpreter','latex','FontSize',14)
    text(1.1,1.4,'Low (0)','Interpreter','latex','FontSize',14)
    text(3.7,1.4,'Medium (0.5)','Interpreter','latex','FontSize',14)
    text(7.2,1.4,'High (1)','Interpreter','latex','FontSize',14)

    LegI = legend('Location','northwest','Interpreter','latex','FontSize',14,'NumColumns',1,'Box','off');    
    ...
    for i = Idx
        ...
        if i == Idx(1)
            ax = nexttile([height_Pd1,width_Pl]);
            ax.Visible = 'off';
        elseif i == Idx(3)
            ax = nexttile([Tl_Dim{1},width_Sp]);
            ax.Visible = 'off';
        else
            ax = nexttile([height_Pd,width_Pl]);
            ax.Visible = 'off';
        end

        if i == Idx(4)
            LegI.Position(2) = LegI.Position(2) + 0.025;
        end

        %%

        ax = nexttile([height_Iq,width_Pl]);
        hold on, grid on, box on
        ...
        if ~isempty(args.Xl_XData)
            xline(args.Xl_XData,args.Xl_LineSpec,args.Xl_Label, ...
                ...
                "Interpreter","latex","FontSize",13,"HandleVisibility","off");
        end
...
function make_title(~,str)
    title(TeX("\makebox[8cm]{" + str + "\hfill}"),"FontSize",14,"Interpreter","latex")
end
