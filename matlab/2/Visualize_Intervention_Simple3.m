...
% 
function [fig,ret] = Visualize_Intervention_Simple3(R,XData,args)
arguments
    R
    XData = (0:height(R)-1)'
    args.FigDim = [905 616]
...
XTicks = XData(1:20:end);
XTicks(end) = XData(end);
...
if ~isempty(args.TrRateStd)
    TrRate = R.(args.TrRateVarName);
    PlMean = plot(XData,TrRate,'.k');

    XXXData = [XData, XData, XData]';
    XXXData = XXXData(2:end);

    YYYData = [TrRate-a*R.(args.TrRateStd),TrRate+a*R.(args.TrRateStd),TrRate+NaN]';
    YYYData = YYYData(1:end-1);

    plot(XXXData,YYYData,'_-k','MarkerSize',2)
...
make_title('Estimated transmission rate $\pm$\,2std ($\approx 95\%$\,CI)')
