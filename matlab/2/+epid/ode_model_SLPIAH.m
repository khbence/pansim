function [f,h,hvar,J] = mf_epid_ode_model_SLPIAH(Np)
arguments
    % Population Hungary
    Np = 9709786
...
matlabFunction(f_sym,'File','Fn_SLPIAH_ode','Vars',{x,p,beta,nu});
matlabFunction(h_sym,'File','Fn_SLPIAH_out','Vars',{x,p,beta});

Ts = 1;

f = {};
f.desc = 'f(x,u,p,v)';
f.val = x_Cas + Ts*Fn_SLPIAH_ode(x_Cas,p_Cas,beta_Cas,nu_Cas);
f.Fn = Function('f',{x_Cas,p_Cas,beta_Cas,nu_Cas},{f.val},{'x','p','beta','nu'},{f.desc});

f.      Input_1 = x;
f.desc__Input_1 = 'State vector (x = [S,L,P,I,A,H])';
f.      Input_2 = p;
...
h.val = Fn_SLPIAH_out(x_Cas,p_Cas,beta_Cas);
