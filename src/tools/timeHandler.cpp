...
Timehandler::Timehandler(unsigned timeStep_p,
    unsigned weeksInTheFuture,
    Days _startDay,
    unsigned _startDate)
    ...
    dayOffset = static_cast<unsigned>(getDay()) > static_cast<unsigned>(startDay)
                    ? (static_cast<unsigned>(startDay) + 7) - static_cast<unsigned>(getDay())
                    : static_cast<unsigned>(startDay) - static_cast<unsigned>(getDay());
}
...
unsigned HD Timehandler::getMinutes() const { return (counter % stepsPerDay) * static_cast<unsigned>(timeStep.count()); }
...
Days Timehandler::getDay() const {
    ...
    // date->tm_wday = date->tm_wday==0 ? 6 : date->tm_wday - 1;
    return static_cast<Days>((static_cast<unsigned>(date->tm_wday) + dayOffset) % 7);
}