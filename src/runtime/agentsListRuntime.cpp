...
std::map<unsigned, unsigned> AgentList<PPState, AgentMeta, Location>::initAgentTypes(const parser::AgentTypes& inputData) {
...
template<typename PPState, typename AgentMeta, typename Location>
void AgentList<PPState, AgentMeta, Location>::initializeArgs(const cxxopts::ParseResult& result) {
...
template<typename PPState, typename AgentMeta, typename Location>
void AgentList<PPState, AgentMeta, Location>::initAgents(
    parser::Agents& inputData,
    ...
    for (auto& person : inputData.people) {
        if (disableTourists && person.typeID == 9) {
            continue;
        }
        ...
        AgentStats stat;
        if (PPValues_h.back().isInfected() > 0) {
        ...
        for (const auto& l : person.locations) {
            ...
            auto itRequested = std::find(requestedLocs.begin(), requestedLocs.end(), l.typeID);
            if (itRequested != requestedLocs.end()) {
                hasThatLocType[std::distance(requestedLocs.begin(), itRequested)] = true;
            }
        }
        try {
        ...
        auto itHome = std::find(ts.begin(), ts.end(), homeType);
        if (itHome == ts.end()) {
            location_h.push_back(0);
        } else {
            location_h.push_back(locs[std::distance(ts.begin(), itHome)]);
        }
    }
...
template<typename PPState, typename AgentMeta, typename Location>
void AgentList<PPState, AgentMeta, Location>::printAgentStatJSON(const std::string& fileName) {
    ...
    if (diagnosticLevel > 0) {
        ...
        unsigned cemeteryLocation = Location::getInstance()->locType.size() - 1;
        ...
        thrust::copy(
            thrust::make_permutation_iterator(
                Location::getInstance()->locType.begin(), thrust::make_transform_iterator(agentStats.begin(), lambda)),
            thrust::make_permutation_iterator(
...
using SimulationT = config::Simulation_t;
template class AgentList<typename SimulationT::PPState_t, typename SimulationT::AgentMeta_t, typename SimulationT::LocationType>;
